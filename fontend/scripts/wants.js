...
const wantAmount = document.querySelector(".wantAmount");

submit.addEventListener("click", (e) => {
  let wantBalance = Number(wantAmount.textContent.split(" ")[2]);
  ...
  if (
    ...
    Swal.fire({
      icon: "error",
      title:
        "opps!😨 enter all field while creating items for Wants budgeting💸",
    });
  ...
  } else if (wantBalance < Number(Price.value)) {
    Swal.fire({
      icon: "info",
...
async function display() {
  ...
  let needsAmount = document.querySelector(".wantAmount");
  needsAmount.textContent = "";
  let fetch = await axios.get("/home/allocation");
  let Needs = fetch.data.Wants;
...
async function displayCategory() {
  ...
  for (let e of data) {
    ...
    a.href = "";
    a.innerHTML = `<i class="fa-solid fa-pencil updateCategoryW" id="${e.categoryName}" style="color: #000000;"></i>`;
    ...
    innerArray.forEach((element) => {
      ...
      let a = document.createElement("a");
      a.href = "";
      a.innerHTML = `<i class="fa-solid fa-pencil updateWantItem" id="${element.itemName},${element.itemprice}" style="color: #000000;font-size:small"></i>`;
...
//to check if e.target.classList.contain="updateCategoryW";
document.addEventListener("click", (e) => {
  if (e.target.classList.contains("updateCategoryW")) {
    e.preventDefault();
